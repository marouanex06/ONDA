...
export const createBonSortie = createAsyncThunk(
  ...
  async (bonSortie, { rejectWithValue, getState }) => {
    try {
      ...
      const response = await axiosClient.post("?endpoint=createBS", {
        code_bs: bonSortie.code_bs || "",
        id_demande: Number(bonSortie.id_demande),
        date_sortie:
          bonSortie.date_sortie || new Date().toISOString().split("T")[0],
        nb_sortie: Number(bonSortie.nb_sortie),
      });

      if (!response.data?.success) {
        return rejectWithValue(response.data?.message || "Erreur lors de la creation");
      }
